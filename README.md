...
自社情報のロゴ画像保存には Vercel Blob を使用します。Vercelのプロジェクト →「Storage」→「Create Database」→「Blob」でストアを作成しプロジェクトに接続すると、認証情報が自動的に設定されます（手動設定は不要）。接続方式によって `BLOB_READ_WRITE_TOKEN` が付与される場合と、`BLOB_STORE_ID` ＋ 実行時に自動注入される `VERCEL_OIDC_TOKEN` によるOIDC認証になる場合があり、いずれの方式でもコード変更なく動作します。
