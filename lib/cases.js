...
// 保持する検索履歴の件数。案件詳細モーダルは検索履歴だけの専用スクロール領域を
// 持たず、モーダル全体(.modal-sheet、max-height:85vh)のスクロールに乗っている
// ため、無制限に増えると他のセクション(カート内容・見積書ボタン等)まで
// スクロールしないと辿り着けなくなる。RPUSH(末尾に追記)のたびにLTRIMで直近
// この件数だけを残す。
const MAX_SEARCH_HISTORY_ENTRIES = 12;

export async function addSearchToCase(caseId, searchData) {
  const key = searchesKey(caseId);
  const pipeline = redis.pipeline();
  pipeline.rpush(key, JSON.stringify({ ...searchData, searchedAt: Date.now() }));
  // LTRIMの範囲は末尾基準の負インデックス。RPUSHは末尾に追加するため、
  // 直近MAX_SEARCH_HISTORY_ENTRIES件(末尾からその件数分)だけを残す。
  pipeline.ltrim(key, -MAX_SEARCH_HISTORY_ENTRIES, -1);
  await pipeline.exec();
}
