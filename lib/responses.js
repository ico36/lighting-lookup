...
/**
 * プラン限定機能(lib/featureCampaigns.js)を、許可されていないプランで使おうとしたときの
 * 403ボディ。
 *
 * 【limitsキーを使わない理由】caseLimitExceededBody() の limits は
 * { plan, used, limit }(件数の上限とその消費量)という別の性質のデータで、キー名を
 * 揃えると混同しやすい。この関数が返すのは「その機能を使えるプランかどうか」だけなので、
 * 中身の形も別にしてある。
 *
 * 【messageに具体的なプラン名を含めない理由】工程E-1の決定(識別子→日本語名の対応表を
 * コードに持たない。利用者に見せるプラン表示名はStripeのProduct名を使う)に沿い、
 * ここでは「上位プラン」という抽象的な言い方に留める。具体的なプラン名の表示は
 * アップグレードモーダル側(Stripeから取得したProduct名を使う経路)に委ねる。
 *
 * @param {{featureKey: string}} args
 */
export function featureRestrictedBody({ featureKey }) {
  return {
    error: 'feature_restricted',
    feature: featureKey,
    message: 'この機能は上位プランでご利用いただけます。',
  };
}
