 ...
 *
 * @returns {Promise<{customerId: string|null, subscription: object|null, item: object|null}>}
...
export async function getActiveSubscriptionWithItem(email) {
  ...
  if (!customerId) {
    return { customerId: null, subscription: null, item: null };
  }
...
export async function checkActiveSubscriptionLive(email) {
  ...
  const { customerId, subscription, item } = await getActiveSubscriptionWithItem(email);
 ...
 * TTLの1時間使い回すことは構造上起きない。保存する形は { active, limits, period } の
 * ままなので CACHE_SCHEMA_VERSION の更新も不要。
 *
 * @returns {Promise<{active: boolean, limits: {plan: string, searchLimit: number, caseLimit: number, retentionDays: number}, period: {start: number, end: number} | null, degraded: boolean}>}
 */
export async function getSubscriptionStateCached(email) {
  ...
  if (isAdminEmail(email)) {
    ...
    return { active: true, limits: { ...ADMIN_PLAN_LIMITS }, period: null, degraded: false };
  }
  ...
  try {
    ...
    // なおこの経路は下の redis.set を通らないので、劣化した値はキャッシュされない。
    return { active: true, limits: { ...DEFAULT_PLAN_LIMITS }, period: null, degraded: true };
  }
