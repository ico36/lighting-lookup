...
//
// このとき、プレビューで使った proration_date を実際の変更でも渡さないと、提示額と
// 請求額がズレる（Stripeの型定義に明記されている。経過した秒数ぶん日割りが変わる）。
// つまり「移行先」と「その移行先をプレビューしたときの proration_date」は必ず対で
// 扱う必要がある。
//
// 素朴にフロントが { plan, prorationDate } を対で持って送り返す形だと、次の3つを
// 防げない:
//   - 取り違え ... 候補Aの日付で候補Bへ変更してしまう
//   - 改ざん   ... 古い proration_date を送って安く確定させる
//   - 陳腐化   ... モーダルを開いたまま放置し、数日前の日付で確定してしまう
//
...
// 「数日前の日割り」で確定できてしまう。Stripe側も proration_date が現在の請求期間内
// である必要があるため、いずれにせよ長くは持てない。
 ...
 *          prorationDate: number, amountDue: number, currency: string,
 *          couponApplied: boolean}} quote
 *   prorationDate ... 秒エポック（Stripeの単位のまま持つ。この値をそのまま
 *                     subscriptions.update に渡すため、ミリ秒へ変換しない）
 *
 * 【同じオブジェクト内で時刻の単位が違う】prorationDate は秒（Stripeへ渡す値なので
 * Stripeの単位のまま）、expiresAt はミリ秒（Date.now() を使うこのリポジトリの表現）。
 * 意図的な混在だが取り違えやすいので、両方に単位を明記してある。
 *
